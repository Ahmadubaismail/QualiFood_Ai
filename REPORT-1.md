...
|---|---|---|
| Throughput | **31.8 tokens/sec** | `ollama run qwen2.5:1.5b --verbose`, read from `eval rate` in output |
| Peak RAM usage | **2.4 GB** | Windows Task Manager, Performance tab, observed during an active `/chat` request |
| Latency (query → response) | **3.9 seconds** | Stopwatch measurement from pressing "Send" in the Ask AI tab to full response rendering |
| Thermal / throttling | No throttling observed during test session | Manual observation during sustained use |

*These figures were measured on the developer's Windows laptop (Intel/AMD CPU, 8 GB RAM, integrated graphics, no discrete GPU) — matching the ADTC Standard Laptop profile. A throughput of 31.8 tokens/sec comfortably exceeds typical usability thresholds for conversational responses, while 2.4 GB peak RAM usage leaves significant headroom within the 7 GB efficiency budget defined by the ADTC scoring rubric, and the 3.9-second end-to-end latency (retrieval + generation) is well within an acceptable range for an interactive assistant.*
